...
class UtilisateurModel:
    ...
    def create(email, mot_de_passe):
        ...
        Args:
            ...
            mot_de_passe (str): Mot de passe / PIN (à hasher dans une vraie app)
    ...
    def update_by_id(id_utilisateur, email=None, mot_de_passe=None):
        ...
        Args:
            ...
            mot_de_passe (str, optional): Nouveau mot de passe / PIN
    ...
    def verify_pin(pin):
        """
        Vérifier si le PIN correspond au mot_de_passe d'un utilisateur dans la base de données
        
        Args:
            pin (str): Code PIN à vérifier
            
        Returns:
            dict: Dictionnaire contenant l'utilisateur si le PIN est valide, None sinon
        """
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM "Utilisateur" WHERE mot_de_passe = ?', (pin,))
        row = cursor.fetchone()
        conn.close()
    ...
    # reinitialiser les donnees de l'utilisateur
    @staticmethod
    def reinitialiser_donnes():
        try:
            conn = get_connection()
            cursor = conn.cursor()
            ...
            # Supprimer tout l'historique des soldes (remet le solde à zéro)
            cursor.execute('DELETE FROM "Portefeuille"')

            # Réinitialiser les budgets à 0
            cursor.execute('UPDATE "Budget" SET budget_total = 0, jours_restants = 20')
